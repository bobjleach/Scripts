#  
# This script adds an idrac to a discovery range within OME. It
# prompts the user for various information along with global
# variables perfrom this task.
# 
# to execute:
# Add_Device.ps1
...
try{
    ...
    $Idrac_IP = Read-Host "Please enter iDRAC IP Address"
    ...
    $Return_results['IDRAC_IP'] = $Idrac_IP
...
try{
    ...
    if ($Results['RETURN_CODE'] -eq $true){
        ...
        $Msg = "Created discovery task in OME: "+$Results['OME_DISCOVERY_NAME']
        Write-Host $Msg
        ...
        if ($Results['RETURN_CODE'] -eq $true){
            # Completed fine 
            write-host "`nDiscovery tasks successfully completed within OME."

        }
        else{
            # Failed
            Write-Host "`nDiscovery task within OME failed, please try again." 
        }

    }
    else{
        Write-Host "Error occured while creating discovery task in OME, please try again."
    }
}
catch{
    # runtime error 
